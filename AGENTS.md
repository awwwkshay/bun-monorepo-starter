...
- **`apps/web`**: **TanStack Start** application (React 19, Vite, Tailwind v4, TanStack Router). This is the primary user-facing frontend.
- **`apps/api`**: Bun-native API service providing backend functionality and data management.
- **`apps/cli`**: Bun-native command-line interface tool for internal or external tooling tasks.
- **`packages/core`**: Shared business logic, shared types, and common utilities used across apps.
...
### Shared Packages & Other Apps

- **Test All**: `bun test`
- **Run Single Test**: `bun test <path/to/test>`
- **Run by Pattern**: `bun test -t "pattern"`
...
- **Server**: Use `Bun.serve()` instead of Express or other frameworks.
- **File System**: Use `Bun.file()` and `Bun.write()` for better performance.
- **Database**: Use `bun:sqlite` or `Bun.sql`.
...
- To add shared package: `bun add @bun-mono/core --filter @bun-mono/web` (example)
...
4. **Context Awareness**: Check `CLAUDE.md` in sub-directories if they exist for specific package overrides.
5. **No Vite for Pure Bun**: In non-`web` apps, use HTML imports with `Bun.serve()` if frontend is needed, avoiding Vite.

## 7. Additional Rules (Copilot/Cursor)

No specific Cursor rules (`.cursorrules`) or Copilot instructions (`.github/copilot-instructions.md`) found. Rely on the mandates in this file and the root `CLAUDE.md`.
